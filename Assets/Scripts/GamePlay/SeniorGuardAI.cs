...
public class SeniorGuardAI : MonoBehaviour {
	...
	bool ListenForPlayer() {
		distVec = player.transform.position - transform.position;
		return (distVec.magnitude < hearingDist
			&& Physics.Raycast(transform.position, distVec, out hit, hearingDist) && hit.collider.gameObject.tag == "Player");
	}
